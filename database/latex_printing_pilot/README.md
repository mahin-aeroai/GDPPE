...
**2. Schema gap found and now RESOLVED: `UVCuringType` didn't fit latex.** UV Printing's Template 1 has a "UV Curing" section with a vocabulary literally named `UVCuringType` (originally LED | Mercury | Hybrid | UV_Gel | Not_Applicable). Latex doesn't cure with UV light at all — it dries via radiant heat and forced airflow evaporating water from the ink. This pilot used a new `drying_technology` free-text field for latex machines rather than force a `Not_Applicable` into a field asking the wrong question.

**Resolution**: per direct domain correction, `UVCuringType` is now simplified to exactly `Arc_Lamp | LED_Lamp` — the two real UV curing mechanisms, with `Mercury`/`Hybrid`/`UV_Gel`/`Not_Applicable` removed as category errors in the original vocabulary (see `schema/schema.md`'s v0.20 decision record for the full reasoning). The vocabulary stays narrowly scoped to actual UV curing; `drying_technology` remains its own separate field for latex rather than being merged in. This is the right outcome — UV curing and thermal drying are different enough physical processes that one shared vocabulary was never going to fit both cleanly.
