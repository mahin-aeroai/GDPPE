...
| `SublimationWorkflow` | Transfer \| Direct \| Dual | Template (Dye-Sublimation Printing) "Print System" section. How the sublimation image reaches the fabric: `Transfer` (print to paper, then heat-press/calender onto polyester), `Direct` (print straight onto fabric, fix inline or separately), `Dual` (machine does both). This is dye-sub's structurally distinguishing axis - the ink chemistry is uniformly disperse-dye, so workflow, not ink, defines the machine classes. Note: pure `Direct`-only machines are rare (transfer remains the flexible default), so direct-capable machines are usually `Dual`; the `Direct` value exists for future machines. Durst iSUB adds inline (contactless) fixation, folding the separate calender stage into the printer. |
| `InkSubtype` | Dye \| Pigment | Template (Aqueous / Inkjet Printing) "Ink System" section. The fundamental split within water-based inks: `Dye` (dissolved colourant - wider gamut, lower cost, less light-fast; now mostly desktop photo) vs `Pigment` (suspended particles - durable, light-fast; dominant in professional/large-format). Distinct from `InkType`, which records the broad chemistry family (Aqueous/Solvent/UV/Latex/Dye_Sublimation); `InkSubtype` refines the aqueous family specifically. |
| `PrintArchitecture` | Scanning_Carriage \| Pagewide_Array | Template (Aqueous / Inkjet Printing) "Print System" section. How the printhead covers the page: `Scanning_Carriage` (head traverses back and forth across the media - the near-universal default) vs `Pagewide_Array` (a stationary printhead spans the full media width; media advances under it in a single pass - far faster, used in production technical printers e.g. HP PageWide XL). A structural machine-class axis, applicable beyond aqueous but first modelled here. |
| `AutomationLevel` | Manual \| Automatic | Template (Screen Printing) "Print System" section. Whether the press runs under operator power or motor control: `Manual` (operator rotates the carousel and pulls each squeegee stroke by hand) vs `Automatic` (servo/chain-driven indexing and printing). Screen printing's defining structural axis — and the first category to need it, since every inkjet and toner machine is inherently automatic. Applicable to any process with a hand-operated tier. |
| `SheetFormat` | B3 \| A1 \| A1_Plus \| B1 (extensible: B2, B0 ...) | Template (Offset Litho) "Mechanical" section. The sheet size class that defines a sheetfed press, the primary way offset machines are segmented commercially. `B3` (~35x50 cm compact), `A1` (~64x94 cm), `A1_Plus` (RMGT's '8-up' bridge, ~65x96.5 cm), `B1` (~70-75x100-106 cm large format). Extensible to B2/B0 and beyond as machines are added. Paired with `max_sheet_size` for the exact dimensions. No inkjet/toner/screen category needed a sheet-format class — those are rated by print width or image area instead. |
