# GDPPE Entity Schema v0.20
### Global Digital Print Production Equipment Encyclopedia — Data Model
*v0.2: Ch.2 research methodology. v0.3: Ch.3 classification taxonomy. v0.4: Ch.4 Manufacturer Directory. v0.5: Ch.5 Master Equipment Index. v0.6: Ch.6 physical architecture. v0.7: Ch.7 source standards. v0.8: Ch.8 Technology-Specific Templates. v0.9: Ch.9 Data Dictionary. v0.10: Ch.10 Engineering Data Collection Standards. v0.11: Ch.11 HTML IA/UX. v0.12: Ch.12 Page Design Standards. v0.13: Ch.13 Comparison/Benchmarking. v0.14: Ch.14 Knowledge Graph. v0.15: Ch.15 AI Architecture. v0.16: Ch.16 Implementation Roadmap. v0.17: Ch.17 Governance/Ethics. v0.18: Project Roadmap memo. v0.19: Category count decision — CLOSED. v0.20: **`UVCuringType` simplified to `Arc_Lamp | LED_Lamp`, per domain review during Phase 2's Latex pilot.**

> ## UVCuringType: RESOLVED to two values, per direct domain correction
>
> The original 5-value vocabulary (`LED, Mercury, Hybrid, UV_Gel, Not_Applicable`) mixed three different kinds of error: `Mercury` and `LED` described the same axis at different specificity (mercury-vapor is the standard arc-lamp implementation, not a separate mechanism from "arc lamp" generally); `Hybrid` described a machine capability (some machines carry both lamp types) rather than a value for one curing event; and `UV_Gel` is an ink chemistry, not a curing mechanism — it already exists correctly in `InkType` and was a category error to have duplicated here.
>
> **Resolved: `UVCuringType` is now exactly `Arc_Lamp | LED_Lamp`.** A machine with both lamp types gets two `FieldValue` records (one per installed curing unit) rather than a third "Hybrid" enum value — consistent with how the schema handles every other multi-configuration case. `Not_Applicable` is removed from this vocabulary specifically: if UV curing doesn't apply to a machine (e.g. a latex printer, which dries via heat — see `drying_technology`, a separate field flagged during the Latex pilot, not merged into this vocabulary), the field is simply absent for that machine rather than populated with a UV-specific sentinel.
>
> This also resolves — by *not* merging — the open question from the Latex pilot about whether `UVCuringType` should be renamed to a technology-neutral vocabulary covering thermal drying too. The correction here confirms `UVCuringType` should stay narrowly scoped to actual UV curing mechanisms; thermal drying remains its own separate field. If a third category introduces a third distinct curing/drying mechanism, that's a new field for that mechanism, not a growing shared enum.
>
> No pilot data needed correction — every UV Printing machine researched so far (Canon, EFI, Durst, Agfa, Mimaki, swissQprint, Roland, HandTop) was already recorded as `LED`, which maps directly to `LED_Lamp`.
...
| Ch.12 §12.5 Hero Badges (Current Model, Industrial Grade, LED UV, Arc Lamp UV...) | Computed/derived at render time from `machine_status`, `production_class`, `UVCuringType`, `MachineConfiguration` (§3a) — not stored fields of their own |
